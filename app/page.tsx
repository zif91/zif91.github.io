...
const topics = [
  ...
  { name: 'Выгорание', slug: 'burnout' },
  { name: 'Отношения', slug: 'couples' },
  { name: 'Панические атаки', slug: 'panic' },
  { name: 'Нарушения сна', slug: 'sleep' },
];
const filters = ['Все', 'Тревога', 'Отношения', 'Выгорание', 'Зависимость'];
const specialists = [
  { slug: 'maria-sokolova', name: 'Мария Соколова', role: 'Психолог, КПТ-терапевт', tags: ['Тревога', 'Выгорание'], exp: '9 лет опыта', price: 'от 6 000 ₽', tone: 'mint', initials: 'МС' },
  { slug: 'anton-vlasov', name: 'Антон Власов', role: 'Семейный психолог', tags: ['Отношения', 'Выгорание'], exp: '12 лет опыта', price: 'от 7 000 ₽', tone: 'sand', initials: 'АВ' },
  { slug: 'elena-romanova', name: 'Елена Романова', role: 'Клинический психолог', tags: ['Тревога', 'Зависимость'], exp: '15 лет опыта', price: 'от 7 000 ₽', tone: 'rose', initials: 'ЕР' },
];
...
export default function Home() {
  ...
  return (
    <main>
      ...
      <section className="topic-strip" id="directions"><p>Можно начать с того,<br />что беспокоит сейчас</p><div className="topic-list">{topics.map((topic, index) => <a href={`/directions/${topic.slug}`} key={topic.slug}><span>0{index + 1}</span>{topic.name}</a>)}</div></section>
      ...
      <section className="section specialists-section" id="specialists">
        ...
        <div className="specialist-grid">
          {visible.map((doctor) => <article className="specialist-card" key={doctor.name}>
            ...
            <a href={`/specialists/${doctor.slug}`}>Посмотреть профиль <span>↗</span></a>
          </article>)}
        </div>
      ...
      <section className="section service-section">
        ...
        <div className="service-grid">
          <a href="/directions/anxiety" className="service-card coral"><span>01</span><h3>«Я всё понимаю головой, но ничего не меняется»</h3><p>Тревога, апатия, панические атаки, нарушение сна</p><b>Индивидуальная терапия →</b></a>
          <a href="/directions/couples" className="service-card sage"><span>02</span><h3>«Мы любим друг друга, но жить так уже невозможно»</h3><p>Кризис, конфликты, измена, решение о разводе</p><b>Работа с парой →</b></a>
          <a href="/directions/burnout" className="service-card sand"><span>03</span><h3>«Мне нужны конкретные инструменты, а не просто поговорить»</h3><p>Выгорание, раздражительность, потеря мотивации</p><b>Стратегия восстановления →</b></a>
          <a href="/directions/addiction" className="service-card dark"><span>04</span><h3>Помощь при зависимости — анонимно</h3><p>Консультации для самого человека и его близких. Без осуждения и постановки на учёт.</p><b>Узнать о процессе →</b></a>
        </div>
      </section>
      ...
      <section className="section article-section" id="articles">
        ...
        <div className="article-grid">
          <a href="/articles/anxiety-without-reason" className="article featured"><span className="article-label">Тревога · 8 минут</span><h3>Что делать, если тревога приходит «из ниоткуда»</h3><p>Почему тело включает тревогу раньше мыслей и как помочь себе в моменте.</p><b>Читать статью →</b></a>
          <a href="/articles/relationship-crisis" className="article"><span className="article-label">Отношения · 6 минут</span><h3>Мы разговариваем только о быте. Это кризис?</h3><b>Читать →</b></a>
          <a href="/articles/burnout-signs" className="article"><span className="article-label">Выгорание · 7 минут</span><h3>Отдых не помогает: как распознать выгорание</h3><b>Читать →</b></a>
        </div>
      </section>
      ...
      <section className="booking-section" id="booking">
        ...
        <form onSubmit={submitForm}>
          ...
          <label>Что вас беспокоит<select name="topic" defaultValue=""><option value="" disabled>Выберите, если готовы</option>{[...topics.map(item => item.name), 'Зависимость', 'Другое'].map(item => <option key={item}>{item}</option>)}</select></label>
